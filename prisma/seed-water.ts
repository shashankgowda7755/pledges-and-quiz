...
async function main() {
  ...
  // 4. Upsert House Sparrow Quiz (general public — no org association)
  ...
  if (existingQ === 0) {
    ...
    for (let i = 0; i < questions.length; i++) {
      const q = await prisma.question.create({
        data: { quizId: quiz.id, text: questions[i].text, order: i + 1 },
      });
      for (let j = 0; j < questions[i].options.length; j++) {
        await prisma.answerOption.create({
          data: { questionId: q.id, text: questions[i].options[j], isCorrect: j === questions[i].correct, order: j + 1 },
        });
      }
    }
