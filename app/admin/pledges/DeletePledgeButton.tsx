...
// Two-way active toggle. Deactivating keeps the event visible but closes
// entries ("This event is over"); activating re-opens it.
export default function DeletePledgeButton({ slug, isActive }: { slug: string, isActive: boolean }) {
  ...
  const handleToggle = async () => {
    ...
    if (!confirm(`Are you sure you want to ${verb} "${slug}"?`)) return;

    setLoading(true);
    try {
      await fetch(`/api/admin/pledges/${slug}`, {
        method: 'PATCH',
